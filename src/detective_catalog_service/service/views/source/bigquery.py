...
@router.post("/insert")
async def post_new_catalog(request: Request, source_connection_properties: BigQuery):
...
async def update_existing_catalog(request: Request, source_connection_xid: str, source_connection_properties: BigQuery):
    token = Token(access_token=request.headers.get('Authentication'))
    if token_validate(token):
