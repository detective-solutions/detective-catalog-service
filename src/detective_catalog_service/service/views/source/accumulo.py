...
@router.post("/insert")
async def post_new_catalog(request: Request, source_connection_properties: Accumulo):
...
async def update_existing_catalog(request: Request, source_connection_xid: str, source_connection_properties: Accumulo):
    token = Token(access_token=request.headers.get('Authentication'))
    if token_validate(token):
